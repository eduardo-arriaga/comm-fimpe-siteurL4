...
public class KilometersReportExcel {

    private List<KilometersReportRecord> kilometersReportRecords;
    private Sheet sheet;
    private Workbook workbook;
    public static final String REPORT_NAME = "Reporte Kilometros";
    private ExcelManager excelManager;
    private static Logger logger = LoggerFactory.getLogger(KilometersReportExcel.class);

    public KilometersReportExcel(List<KilometersReportRecord> countersReportRecords, Workbook workbook) {
        this.kilometersReportRecords = countersReportRecords;
    ...
    public void createTable() {
        try {
            ...
            logger.error("Error al intentar generar el reporte de envios de Kilometros");
        }
        excelManager.autoSize(KilometersReportRecord.getHeaders().size());
    }

    private void createHeaders() {
        excelManager.addTableTitle("Envios Kilometros", KilometersReportRecord.getHeaders().size());
    }
    ...
    private void createRecords() throws ExcelSheetRowLimitException {
        ...
        for (KilometersReportRecord kilometersReportRecord : kilometersReportRecords) {
            row = sheet.createRow(excelManager.getIndexRow());

            for (int i = 0; i < kilometersReportRecord.getOrderValues().size(); i++) {
                cell = excelManager.getCell(row, i, excelManager.getContentStyle());
                cell.setCellValue(kilometersReportRecord.getOrderValues().get(i));
            }
        }
    }
}
